...
{
    public class SpellbookDamage : UIText
    {
        private string cache;
        private VerPlayer Player => Main.CurrentPlayer.GetModPlayer<VerPlayer>();

        public SpellbookDamage(string text, float textScale = 1, bool large = false) : base(text, textScale, large)
        {
            cache = text;
        }
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            {
                string text = string.Format("{0:P0}", SpellbookUIState.SpellData.Value.Damage.Additive);
                if (cache != text)
                {
                    SetText(text);
                    cache = text;
